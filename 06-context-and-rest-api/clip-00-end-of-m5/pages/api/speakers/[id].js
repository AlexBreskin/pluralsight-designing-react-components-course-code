...
export default async function handler(req, res) {
  ...
  async function putMethod() {
    try {
      ...
      res
        .status(500)
        .send(`PUT /api/speakers/${id} error status: 500 unexpected error`);
      console.log(
        `PUT /api/speakers/${id} error status: 500 unexpected error`,
        error
      );
    }
  }
  async function postMethod() {
    try {
      ...
      res
        .status(500)
        .send(`POST /api/speakers/${id} error status: 500 unexpected error`);
      console.log(
        `POST /api/speakers/${id} error status: 500 unexpected error`,
        error
      );
    }
  }
  async function deleteMethod() {
    try {
      ...
      res
        .status(500)
        .send(`DELETE /api/speakers/${id} error status: 500 unexpected error`);
      console.log(
        `DELETE /api/speakers/${id} error status: 500 unexpected error`,
        error
      );
    }
  }
